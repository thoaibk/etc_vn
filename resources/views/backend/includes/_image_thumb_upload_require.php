...
{% for (var i=0, file; file=o.files[i]; i++) { %}
    <div style="min-height: 230px" class="template-upload col col-lg-6 py-2 image-block">
...
{% for (var i=0, file; file=o.files[i]; i++) { %}
    {% if (file.title != 0) { %}
    <div style="min-height: 70px" class="template-download p-2">
        ...
        <div class="image-body">
            {% if (!file.error) { %}
                <button type="button" class="btn btn-danger image-delete btn-xs delete" data-type="{%=file.delete_method%}" data-url="{%=file.delete_url%}"{% if (file.deleteWithCredentials) { %} data-xhr-fields='{"withCredentials":true}'{% } %}>
                    <i class="fa fa-times pointer"data-toggle="tooltip" data-trigger="hover focus" data-placement="top" data-html="true" data-container="body"  title="Xóa "></i>
                </button>

                <img class="image-thumb img-fluid" src="{%=file.url_thumb%}">
